...
- Debian 12 (Bookworm) — tested and working with both installers
- Debian 13 (Trixie) — not tested yet.  only `install-freeswitch.sh` (from source); SignalWire's package repo for trixie is not yet populated
- Ubuntu 22.04 / 24.04 — not tested yet; the token installer is best-effort (set `CODENAME=bookworm`)
...
| --- | --- | --- | --- |
| Debian 12 (Bookworm) | ✅ | ✅ | ✅ |
| Debian 13 (Trixie) | ⚠️ needs pcre2 patch + master branch | ❌ repo not populated | ✅ |
| Ubuntu 22.04 | ✅ | ⚠️ best-effort with `CODENAME=bookworm` | ✅ |
| Ubuntu 24.04 | ✅ | ⚠️ best-effort with `CODENAME=bookworm` | ✅ |
